...
export default function TradeModal({ isOpen, onClose, track }: TradeModalProps) {
  ...
  // 1. 데이터 조회 (Thirdweb useReadContract)
  // 내 보유량
  const { data: mySharesVal, refetch: refetchShares } = useReadContract({
  ...
  const { data: buyPriceVal } = useReadContract({
    contract: stockContract,
    method: "getBuyPrice",
    params: [tokenIdBigInt, amountBigInt]
  });
  ...
  const { data: allowanceVal, refetch: refetchAllowance } = useReadContract({
    contract: tokenContract,
    method: "allowance",
    params: [address || "0x0000000000000000000000000000000000000000", UNLISTED_STOCK_ADDRESS]
  });
  ...
  // 초기화
  useEffect(() => { if (!isOpen) { setAmount('1'); setMode('buy'); } }, [isOpen]);
  ...
  const handleApprove = () => {
    const transaction = prepareContractCall({
        contract: tokenContract,
        method: "approve",
        params: [UNLISTED_STOCK_ADDRESS, BigInt(1000000 * 1e18)]
    });
    sendTransaction(transaction, {
        onSuccess: () => { toast.success("승인 완료!"); refetchAllowance(); },
        onError: (err) => toast.error("승인 실패")
    });
  };

  const handleTrade = () => {
    if (!address) return toast.error("지갑 연결 필요");
    if (Number(amount) <= 0) return toast.error("1주 이상 입력하세요.");

    if (mode === 'buy') {
        if (estimatedCost > myMldBalance) return toast.error("MLD 잔고 부족");
        ...
        if (Number(amount) > myShares) return toast.error("보유 주식 부족");
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-zinc-800 w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-10 duration-200 relative">
        ...
        <div className="p-6 space-y-6">
            <div className="bg-zinc-950 p-4 rounded-2xl border border-zinc-800 space-y-3">
                <div className="flex justify-between text-xs font-bold text-zinc-500 px-1">
                    <span>Amount</span>
                    <span className="text-zinc-400">Owned: {myShares.toFixed(2)}</span>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button 
                        ...
                        className="w-12 h-12 flex-shrink-0 rounded-xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-xl hover:bg-zinc-700 hover:text-white transition active:scale-95 text-zinc-400"
                    >
                        -
                    </button>
                    
                    {/* 입력창 (flex-1로 남은 공간 꽉 채우되, min-w-0으로 넘침 방지) */}
                    <input 
                        type="number" 
                        value={amount} 
                        onChange={(e) => setAmount(e.target.value)} 
                        className="flex-1 min-w-0 bg-black border border-zinc-700 rounded-xl py-3 px-2 text-center text-2xl font-black text-white focus:outline-none focus:border-green-500 transition placeholder:text-zinc-700"
                        placeholder="0"
                    />
            ...
            {needsApproval ? (
                <button onClick={handleApprove} disabled={isPending} className="w-full py-4 rounded-xl font-bold text-lg bg-blue-600 text-white hover:scale-[1.02] transition shadow-lg flex items-center justify-center gap-2">{isPending ? <Loader2 className="animate-spin"/> : '1. Approve MLD'}</button>
            ) : (
                <button onClick={handleTrade} disabled={isPending} className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 transition shadow-lg ${mode==='buy' ? 'bg-green-500 text-black' : 'bg-red-600 text-white'}`}>{isPending ? <Loader2 className="animate-spin"/> : (mode === 'buy' ? 'Confirm Buy' : 'Confirm Sell')}</button>
            )}
        </div>
      </div>
    </div>
