...
export default function TradeModal({ isOpen, onClose, track }: TradeModalProps) {
  ...
  const handleTrade = () => {
    ...
    if (mode === 'buy') {
      if (estimatedCost > myMldBalance) return toast.error("Insufficient MLD balance.");
      ...
      if (Number(amount) > myShares) return toast.error("Not enough shares to sell.");
      ...
      sendTransaction(transaction, {
        ...
        onError: () => toast.error("Sell failed.")
      });
    }
  };
    ...
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-zinc-800 w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-10 duration-200 relative">
        <div className="p-5 border-b border-zinc-800 flex justify-between items-center bg-zinc-950">
          ...
          <button onClick={onClose} className="p-2 bg-zinc-800 rounded-full hover:bg-zinc-700 transition text-zinc-400 hover:text-white" aria-label="Close">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex p-2 gap-2 bg-zinc-900">
          <button
            ...
            className={`flex-1 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition ${
              mode === 'buy' ? 'bg-green-500 text-black' : 'bg-zinc-800 text-zinc-500 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition ${
              mode === 'sell' ? 'bg-red-500 text-white' : 'bg-zinc-800 text-zinc-500 hover:text-white'
            }`}
          >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-zinc-950 p-5 rounded-2xl space-y-3 border border-zinc-800">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-mono text-zinc-300">
                {mode === 'buy' ? estimatedCost.toFixed(4) : estimatedPayout.toFixed(4)} MLD
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-zinc-500">Fees (5%)</span>
              <span className="font-mono text-zinc-600">
                {(mode === 'buy' ? estimatedCost * 0.05 : estimatedPayout * 0.05).toFixed(4)} MLD
              </span>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`font-mono font-black text-xl ${mode === 'buy' ? 'text-green-400' : 'text-red-400'}`}>
                {(mode === 'buy' ? estimatedCost * 1.05 : estimatedPayout * 0.95).toFixed(4)} MLD
              </span>
            </div>
          </div>
          ...
          {needsApproval ? (
            ...
            >
              {isPending ? <Loader2 className="animate-spin" /> : '1. Approve MLD'}
            </button>
          ) : (
            <button
              onClick={handleTrade}
              ...
              className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 transition shadow-lg ${
                mode === 'buy' ? 'bg-green-500 text-black' : 'bg-red-600 text-white'
              }`}
            >
              {isPending ? <Loader2 className="animate-spin" /> : (mode === 'buy' ? 'Confirm Buy' : 'Confirm Sell')}
            </button>
          )}
        </div>
      </div>
