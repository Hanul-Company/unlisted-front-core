...
export default function RentalModal({ isOpen, onClose, onConfirm, isLoading }: RentalModalProps) {
  const [selectedPlan, setSelectedPlan] = useState<number>(6); // 기본값 6개월
      ...
      <div className="bg-zinc-900 border border-zinc-800 w-full max-w-sm rounded-3xl p-6 shadow-2xl relative">
        ...
        <div className="flex justify-between items-center mb-6">
            <div>
                ...
                <p className="text-xs text-zinc-400">대여 기간을 선택하세요.</p>
            </div>
            ...
            {plans.map((plan) => (
                ...
                >
                    <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${selectedPlan===plan.months ? 'bg-blue-500 text-white' : 'bg-zinc-800'}`}>
                            <plan.icon size={16}/>
                        </div>
